...
void LoadData (vector<SubName>& reports) {
    ifstream file(fileName+filePath);
    if (file.is_open()) {
        ...
        cerr << "'BaoCao_HocTap.txt' se duoc tao tai noi ban mo app nay" << endl;
        cerr << "Neu ban muon luu du lieu tai 1 noi nao khac, ban hay chinh lai 'filePath' tai '#define filePath' trong source code" << endl;
    }
    SetConsoleTextAttribute(hConsole, 7);
}
